...
進行中の方針: 「口パク」を廃し「**音楽に反応して踊る／演出する**」アバターへ改造する計画がある。設計議論は `handover-notes.md` に集約。統合版に残した `AnalyserNode` 配線（音量解析）を流用し、出力先を「口の開閉」から「モーション/演出」へ差し替えるのが核（口パク出力は既に無効化済み＝配線だけ残した状態）。
...
- `AnalyserNode` の RMS 音量 → エンベロープ追従（attack 速い/release 可変）を `env` に保持し、現状は音量メーター表示にのみ使用（口パク出力は無効）。`makeAudioEngine()` がマイクと音声ファイル両対応の音声エンジン。「踊る/演出版」ではこの `env` をモーション量へ流用する想定。
