...
進行中の方針: 「口パク」を廃し「**音楽に反応して踊る／演出する**」アバターへ改造中。設計議論は `handover-notes.md` に集約。**踊る版 v1（最小コア）は実装済み**＝読み込んだ曲のビートに**テンポ同期**してキャラが跳ね、**強い拍では大きく跳ね（跳びポ）**、音符パーティクルを飛ばす。詳細は下記「音楽反応（踊る版 v1）」。
...
### 音楽反応（踊る版 v1）
`avatar-app.jsx` のメインループ内に実装。**最小コア**＝テンポ同期バウンド＋強拍アクセント＋音符パーティクル（ビート駆動の25方向首振りは「標準」段階で先送り、口ずさみも無し＝A/Dのみ）。
