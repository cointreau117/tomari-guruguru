...
4. 画面下中央の **♪ 音楽ファイルを読み込む** で曲を読み込むと自動再生され、**ビートにテンポ同期して跳ねて音符が飛びます**。曲のテンポにロックするので、低域が薄い区間でもリズムを保って跳ね続けます
5. サビやキメなど**強い拍では、要所で大きく跳ねます（跳びポ）**
6. 踊り出して少し経つと**表情が笑顔に変わります**。ずっと笑顔だと単調なので、適度に通常顔も差し込まれます。静かなブレイクや停止では通常顔に戻ります
7. 再生音量はプレーヤー（左下）で調整。音量を変えても踊りのタイミング・高さは変わりません（音量メーター表示のみ追従）
