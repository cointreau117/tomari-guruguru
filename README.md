...
ぐるぐる版とトーク版は1つのアプリに統合済み。さらに「**音楽に反応して踊る**」機能の最小コアを実装済みで、読み込んだ曲のビートに**テンポ同期**してキャラが跳ね、**強い拍では大きく跳ね（跳びポ）**、音符パーティクルを飛ばします。
...
6. 再生音量はプレーヤー（左下）で調整。音量を変えても踊りのタイミング・高さは変わりません（音量メーター表示のみ追従）
...
- ビート感度 / 跳ねる強さ / 強拍アクセントの頻度 / 強拍アクセントの大きさ / 音符パーティクル
