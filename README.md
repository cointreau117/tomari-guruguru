...
ぐるぐる版とトーク版は1つのアプリに統合済み。さらに「**音楽に反応して踊る**」機能の最小コアを実装済みで、読み込んだ曲のビートに合わせてキャラが跳ね、音符パーティクルを飛ばします。
...
4. 左下の **♪ 音楽ファイルを読み込む** で曲を読み込むと自動再生され、**ビートに合わせて跳ねて音符が飛びます**
5. 再生音量はプレーヤー（左下）で調整。音量を変えても踊りのタイミング・高さは変わりません（音量メーター表示のみ追従）
